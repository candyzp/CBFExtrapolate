...
void preSlopeCollision(PlayerObject *player, float dt, GameObject *slope) {
  ...
  if (unk1) {
    if ((!player->m_isGoingLeft || player->m_isPlatformer ||
      ...
      cocos2d::CCRect newPlayerRect = {playerSpeed, unk3 + slopeRect.origin.y,
                                       1.0, heightDiff};
      someBool = true;
  ...
  for (int i = 0; i < safeObjectCount; i++) {
    ...
    case GameObjectType::Slope:
      if (!player->m_isSideways) {
        ...
        player->handleRotatedCollisionInternal(dt, object, emptyRect, false,
                                               false, true);
      }
...
void triggerObject(EffectGameObject *object, GJBaseGameLayer *pl,
  ...
  case 2066: {
    ...
    bool isP2 = bot->trajectory().unsafeInner()->m_fakePlayer2 == player;
  ...
  case 2900: {
    RotateGameplayGameObject *rotateObj = (RotateGameplayGameObject *)object;
