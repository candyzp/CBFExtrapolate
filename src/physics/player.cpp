...
void stopDashing(PlayerObject *player) {
  ...
  if (player->m_isPlatformer) {
    cocos2d::CCPoint dashLoc = {(float)player->m_dashX, (float)player->m_dashY};
...
void ringJump(PlayerObject *player, RingObject *ring) {
  ...
  if (!player->m_stateRingJump2)
    return;
  if (player->m_isDashing)
    return;
  if (!player->m_stateJumpBuffered)
    return;
  ...
  if ((player->m_touchedRing || isCustomOrTeleportRing) &&
      (player->m_touchedCustomRing ||
       ring->m_objectType != GameObjectType::CustomRing)) {
    if (player->m_touchedGravityPortal)
      return;
  ...
  if (ring->m_objectType == GameObjectType::TeleportOrb) {
    ...
    teleportPlayer(player->m_gameLayer, (TeleportPortalObject *)ring, player);
...
void updateJump(PlayerObject *player, float dt) {
  ...
  if (player->m_isPlatformer && player->m_isOnSlope && slopeAngleAbs >= 80.0 &&
      player->m_currentSlope->m_unk4F8) {
    player->m_isOnGround = false;
  }
  ...
  if (player->m_isShip || player->m_isBird || player->m_isDart ||
    ...
    if (player->m_isDart) {
      ...
      player->setYVelocity(player->m_playerSpeed * player->m_speedMultiplier *
                               gravityMult * 8.0 * jumpBufferedMult,
                           0);
      ...
      int gravityMult = player->m_isUpsideDown ? -1 : 1;
      velocity =
          player->m_yVelocity - (gravityMult * dt * gravity * origGravity);
        ...
        if (!gt) {
          ...
          if (player->m_isOnSlope || player->m_wasOnSlope) {
            if (player->m_slopeVelocity > 0) {
              ...
              player->setYVelocity(
                  player->m_slopeVelocity * 0.5 + player->m_yVelocity, 0);
              player->setYVelocity(
                  fmin(player->m_yVelocity, prevVelocity * 1.4), 0);
            }
          }
        }
      }
      ...
      if (player->m_jumpBuffered) {
        if (!player->m_isAccelerating) {
          ...
          float fallingMod = phys::playerIsFalling(player) ? 0.5 : 0.4;
        ...
        float fallingMod =
            phys::playerIsFalling(player) && player->m_jumpBuffered ? 0.5 : 0.4;
        float upsideDownMod = player->m_isUpsideDown ? -1 : 1;
        player->setYVelocity(-(upsideDownMod * dt * fallingMod * origGravity) /
                                     vehicleSizeRelated +
                                 player->m_yVelocity,
                             0);
    ...
    if (!player->m_isAccelerating && !player->m_isDart) {
      ...
      if (!player->m_isUpsideDown) {
        player->setYVelocity(
            fmax(player->m_yVelocity, what * -8.0 / vehicleSizeRelated), 0);
...
void togglePlayerScale(PlayerObject *player, bool smallSize) {
  if (player->m_vehicleSize == 1.0 && !smallSize)
    return;
  if (smallSize) {
    if (player->m_vehicleSize != 1.0)
      return;
